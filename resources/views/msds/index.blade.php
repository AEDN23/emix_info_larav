...
@section('content')
    <div class="card">
        ...
        <div class="table-container">
            <table id="msdsTable">
                ...
                <tbody>
                    @foreach($msds_list as $msds)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $msds->nomer_msds }}</td>
                            <td>{{ $msds->nama_msds }}</td>
                                ...
                                <div style="display: flex;">
                                    ...
                                    @if($msds->file)
                                        <button type="button" class="btn-action btn-pdf" title="Lihat PDF"
                                            onclick="viewPDF('{{ asset('public/storage/' . $msds->file) }}', '{{ $msds->nama_msds }}')">
                                    ...
                                    @if($msds->video && $msds->video !== '-')
                                        <button type="button" class="btn-action btn-video" title="Lihat Video"
                                            onclick="viewVideo('{{ asset('public/storage/' . $msds->video) }}', '{{ $msds->nama_msds }}')">
    ...
    <div id="pdfModal" class="modal-overlay">
        <div class="modal-container">
            <div class="modal-header" style="display: block; border-bottom: 2px solid #eee; padding: 15px 25px; background: #fff;">
                <h4 style="margin: 0; font-weight: bold; color: #000; font-size: 1.1rem;">DETAIL MSDS</h4>
                <h4 id="pdfTitle" style="margin: 5px 0; font-weight: bold; color: #000; font-size: 1.1rem;">NAMA MSDS : </h4>
    ...
    <div id="videoModal" class="modal-overlay">
        <div class="modal-container">
            <div class="modal-header" style="display: block; border-bottom: 2px solid #eee; padding: 15px 25px; background: #fff;">
                <h4 style="margin: 0; font-weight: bold; color: #000; font-size: 1.1rem;">DETAIL MSDS</h4>
                <h4 id="videoTitle" style="margin: 5px 0; font-weight: bold; color: #000; font-size: 1.1rem;">NAMA MSDS : </h4>
...
@section('scripts')
    <script>
        ...
        function viewPDF(url, title) {
            document.getElementById('pdfTitle').innerText = 'NAMA MSDS : ' + title.toUpperCase();
            document.getElementById('pdfViewer').src = url;
            document.getElementById('pdfModal').style.display = 'flex';
        }

        function viewVideo(url, title) {
            document.getElementById('videoTitle').innerText = 'NAMA MSDS : ' + title.toUpperCase();
