...
@section('content')
                                ...
                                <div style="display: flex;">
                                    ...
                                    @if($coa->file)
                                        <button type="button" class="btn-action btn-pdf" title="Lihat PDF"
                                            onclick="viewPDF('{{ asset('public/storage/' . $coa->file) }}', '{{ $coa->nama_coa }}')">
                                    ...
                                    @if($coa->video && $coa->video !== '-')
                                        <button type="button" class="btn-action btn-video" title="Lihat Video"
                                            onclick="viewVideo('{{ asset('public/storage/' . $coa->video) }}', '{{ $coa->nama_coa }}')">
    ...
    <div id="pdfModal" class="modal-overlay">
        <div class="modal-container">
            <div class="modal-header" style="display: block; border-bottom: none; padding: 10px 0;">
                <h4 style="margin: 0; font-weight: bold; color: #000; font-size: 1.1rem;">DETAIL CERTIFICATE OF ANALYSIS</h4>
                <h4 id="pdfTitle" style="margin: 5px 0; font-weight: bold; color: #000; font-size: 1.1rem;">NAMA COA : </h4>
    ...
    <div id="videoModal" class="modal-overlay">
        <div class="modal-container">
            <div class="modal-header" style="display: block; border-bottom: none; padding: 10px 0;">
                <h4 style="margin: 0; font-weight: bold; color: #000; font-size: 1.1rem;">DETAIL CERTIFICATE OF ANALYSIS</h4>
                <h4 id="videoTitle" style="margin: 5px 0; font-weight: bold; color: #000; font-size: 1.1rem;">NAMA COA : </h4>
    ...
    <script>
        ...
        function viewPDF(url, title) {
            document.getElementById('pdfTitle').innerText = 'NAMA COA : ' + title.toUpperCase();
            document.getElementById('pdfViewer').src = url;
            document.getElementById('pdfModal').style.display = 'flex';
        }

        function viewVideo(url, title) {
            document.getElementById('videoTitle').innerText = 'NAMA COA : ' + title.toUpperCase();
